...
#----------------------------------------------------------------------------
# Name:         DecryptDialog.py
# Purpose:      Dialog used to decrypt files.
#
# Author:       Félix Medrano Sanz
#
# Created:      
# RCS-ID:       $Id: DecryptDialog.py $
...
[wxID_DIALOG1, wxID_DIALOG1BUTTON1, wxID_DIALOG1BUTTON2, 
 wxID_DIALOG1BUTTONCANCEL, wxID_DIALOG1BUTTONOK, wxID_DIALOG1CHOICE1, 
 wxID_DIALOG1PANEL1, wxID_DIALOG1PANEL2, wxID_DIALOG1PANEL3, 
 wxID_DIALOG1PANEL4, wxID_DIALOG1PANEL5, wxID_DIALOG1PANEL6, 
...
class Dialog1(wx.Dialog):
    ...
    def _init_ctrls(self, prnt):
        ...
        self.staticText1 = wx.StaticText(id=wxID_DIALOG1STATICTEXT1,
              label=_(u'Encrypted file'), name='staticText1',
        ...
        self.staticText4 = wx.StaticText(id=wxID_DIALOG1STATICTEXT4,
              label=_(u'Decrypted file'), name='staticText4',
    ...
    def OnText1Input(self, event):
        ...
        if text1 and os.path.isfile(text1) and (not text2):
            if '.' in Utilities.BasenameMod(text1):
    ...
    def OnOK( self, event ):
        ...
        if not nomAux1:
            Utilities.ErrorDialog(
                _(u'Input the name of the file that you want to decrypt.'),
                _(u'Missing parameters'))
            return False
        # If the file name is empty we can't continue
        if not nomAux2:
            Utilities.ErrorDialog( 
                _(u'Input the name of the decrypted file.'),
                _(u'Missing parameters'))
            return False
        ...
        if (nomAux1 == nomAux2):
            Utilities.ErrorDialog( 
                _(u'Encrypted and decrypted files cannot be the same.'))
            return
        ...
        command = [(Globals.OPENSSL_BIN, ['enc','-d',algorithm,'-in',nomAux1,
            '-out',nomAux2], _(u'Decrypting %s') % baseNameAux)]
        try:
            # Change the cursor to BusyCursor  
            self.busy = wx.BusyCursor()
            error, output = Executor.executeOpenssl(command, 
                _(u'Decrypting file'), password)
        finally :
        ...
        if error:
            Utilities.Log(_(u'Can\'t decrypt %s') % baseNameAux + '' \
