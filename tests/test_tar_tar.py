...
"Tarumba's tar backend with tar archive tests"
...
def test_configure():
    ...
    config.set('backends_l_tar_bin', [TAR_BIN])
    test_cleanup()
    test_utils.copy(DIR)
...
def test_add_new():
    ...
    test_utils.test_add(ARCHIVE, [DIR], ['-b','tar'])
    test_utils.assert_file_exists(ARCHIVE)

def test_add_existing():
    "Add files to the archive"

    test_utils.test_add(ARCHIVE, [FILE], ['-b','tar'])
    test_utils.assert_file_exists(ARCHIVE)
...
def test_add_path():
    ...
    test_utils.test_add(ARCHIVE, [FILE], ['-b','tar','-p',PATH,'-d'])
    test_utils.assert_file_exists(ARCHIVE)

def test_add_links():
    "Add links to the archive"

    test_utils.test_add(ARCHIVE, [LINK1], ['-b','tar'])
    test_utils.assert_file_exists(ARCHIVE)

def test_add_links_follow():
    "Add links to the archive"

    test_utils.test_add(ARCHIVE, [LINK2], ['-b','tar','-f'])
...
def test_list_one():
    ...
    test_utils.test_list(ARCHIVE, [FILE], ['-b','tar'])

def test_list_all():
    "List all contents"

    test_utils.test_list(ARCHIVE, [], ['-b','tar'])

def test_list_columns():
    "List all contents with custom columns"

    test_utils.test_list(ARCHIVE, [], ['-b','tar','-c','NAME'])

def test_extract_one():
    "Extract one file from the archive"

    test_utils.test_extract(ARCHIVE, [FILE], ['-b','tar','-a'])
...
def test_extract_prompt(mocker):
    ...
    test_utils.test_extract(ARCHIVE, [FILE], ['-b','tar'])
    test_utils.assert_file_exists(FILE)

def test_extract_occurrence():
    "Extract one file from the archive with occurrence"

    test_utils.test_extract(ARCHIVE, [FILE], ['-b','tar','-a','-o','1'])
    test_utils.assert_file_exists(FILE)
...
def test_extract_all():
    ...
    test_utils.test_extract(ARCHIVE, [], ['-b','tar','-a'])
...
def test_extract_none():
    ...
    test_utils.test_extract(ARCHIVE, [], ['-b','tar','-n'])
    test_utils.assert_dir_exists(DIR)
    test_utils.assert_file_exists(FILE)
    test_utils.assert_file_exists(os.path.join(PATH, FILE))
    test_utils.assert_link_exists(LINK1)
