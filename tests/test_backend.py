...
class TestBackend:
    ...
    def test_configure(self, test_params):
        ...
        test_utils.test_configure(test_params)
    ...
    def test_add_new(self, test_params):
        ...
        if backend.can_pack():
            test_utils.test_add(test_params.archive, [test_utils.DIR, test_utils.FILE_ABS], ["-b", test_params.backend])
            test_utils.assert_file_exists(test_params.archive)
        else:
            with pytest.raises(SystemExit):
                test_utils.test_add(test_params.archive, [test_utils.DIR], ["-b", test_params.backend])

    def test_add_new_encrypted(self, test_params, mocker):
        "Create the encrypted archive"

        backend = t_classifier.detect_format(
            test_params.backend, test_utils.ENC_PRE + test_params.archive, t_constants.OPERATION_ADD
        )
        if backend.can_encrypt():
            mocker.patch("rich.prompt.Prompt.ask", return_value=test_utils.PASSWORD)
            test_utils.test_add(
                test_utils.ENC_PRE + test_params.archive, [test_utils.DIR], ["-b", test_params.backend, "-e"]
            )
            test_utils.assert_file_exists(test_params.archive)
        else:
            with pytest.raises(SystemExit):
    ...
    def test_add_existing(self, test_params):
        ...
        test_utils.test_add(test_params.archive, [test_utils.FILE1], ["-b", test_params.backend])
        test_utils.assert_file_exists(test_params.archive)
    ...
    def test_add_new_existing_encrypted(self, test_params, mocker):
        ...
        if backend.can_encrypt():
            mocker.patch("rich.prompt.Prompt.ask", return_value=test_utils.PASSWORD)
            test_utils.test_add(
                test_utils.ENC_PRE + test_params.archive, [test_utils.FILE1], ["-b", test_params.backend, "-e"]
            )
            test_utils.assert_file_exists(test_params.archive)
        else:
            with pytest.raises(SystemExit):
    ...
    def test_add_level(self, test_params):
        ...
        test_utils.test_add(test_params.archive, [test_utils.FILE2], ["-b", test_params.backend, "-l", "3"])
        test_utils.assert_file_exists(test_params.archive)
    ...
    def test_add_level_encrypted(self, test_params, mocker):
        ...
        if backend.can_encrypt():
            mocker.patch("rich.prompt.Prompt.ask", return_value=test_utils.PASSWORD)
            test_utils.test_add(
                test_utils.ENC_PRE + test_params.archive,
                [test_utils.FILE2],
    ...
    def test_add_path(self, test_params):
        ...
        if backend.can_pack():
            test_utils.test_add(
                test_params.archive, [test_utils.FILE1], ["-b", test_params.backend, "-p", test_utils.PATH1]
            )
            test_utils.assert_file_exists(test_params.archive)
        else:
            with pytest.raises(SystemExit):
    ...
    def test_add_path_follow(self, test_params):
        ...
        if backend.can_pack():
            test_utils.test_add(
                test_params.archive, [test_utils.FILE1], ["-b", test_params.backend, "-p", test_utils.PATH2, "-k"]
            )
            test_utils.assert_file_exists(test_params.archive)
        else:
            with pytest.raises(SystemExit):
    ...
    def test_add_links(self, test_params):
        ...
        backend = t_classifier.detect_format(test_params.backend, test_params.archive, t_constants.OPERATION_ADD)
        if backend.can_pack():
            test_utils.test_add(test_params.archive, [test_utils.LINK1], ["-b", test_params.backend])
            test_utils.assert_file_exists(test_params.archive)
        else:
            with pytest.raises(SystemExit):
                test_utils.test_add(test_params.archive, [test_utils.LINK1], ["-b", test_params.backend])
    ...
    def test_add_links_follow(self, test_params):
        ...
        test_utils.test_add(test_params.archive, [test_utils.LINK2], ["-b", test_params.backend, "-k"])
    ...
    def test_list_occurrence(self, test_params):
        ...
        if backend.can_duplicate():
            test_utils.test_list(test_params.archive, [test_utils.FILE1], ["-b", test_params.backend, "-o", "1"])
            test_utils.assert_file_exists(test_utils.FILE1)
    ...
    def test_list_encrypted(self, test_params, mocker):
        ...
        backend = t_classifier.detect_format(
            test_params.backend, test_utils.ENC_PRE + test_params.archive, t_constants.OPERATION_LIST
        )
        if backend.can_encrypt():
            mocker.patch("rich.prompt.Prompt.ask", return_value=test_utils.PASSWORD)
            test_utils.test_list(test_utils.ENC_PRE + test_params.archive, [], ["-b", test_params.backend])
    ...
    def test_test_occurrence(self, test_params):
        ...
        backend = t_classifier.detect_format(test_params.backend, test_params.archive, t_constants.OPERATION_EXTRACT)
        if backend.can_duplicate():
            test_utils.test_test(test_params.archive, [test_utils.FILE1], ["-b", test_params.backend, "-o", "1"])
            test_utils.assert_file_exists(test_utils.FILE1)
    ...
    def test_test_encrypted(self, test_params, mocker):
        ...
        backend = t_classifier.detect_format(
            test_params.backend, test_utils.ENC_PRE + test_params.archive, t_constants.OPERATION_TEST
        )
        if backend.can_encrypt():
            mocker.patch("rich.prompt.Prompt.ask", return_value=test_utils.PASSWORD)
            test_utils.test_test(test_utils.ENC_PRE + test_params.archive, [], ["-b", test_params.backend])
    ...
    def test_extract_occurrence(self, test_params):
        ...
        if backend.can_duplicate():
            test_utils.test_extract(
                test_params.archive, [test_utils.FILE1], ["-b", test_params.backend, "-a", "-o", "1"]
            )
            test_utils.assert_file_exists(test_utils.FILE1)
    ...
    def test_extract_all(self, test_params):
        ...
        if backend.can_pack():
            test_utils.assert_dir_exists(test_utils.DIR, archive_folder=test_params.archive)
            test_utils.assert_file_exists(test_utils.FILE1, archive_folder=test_params.archive)
            test_utils.assert_file_exists(
                os.path.join(test_utils.PATH1, test_utils.FILE1), archive_folder=test_params.archive
            )
            test_utils.assert_file_exists(
                os.path.join(test_utils.PATH2, test_utils.FILE1), archive_folder=test_params.archive
            )
            # https://github.com/p7zip-project/p7zip/issues/39
            if test_params.binary not in (test_utils.X7ZA, test_utils.P7ZIP):
                test_utils.assert_link_exists(test_utils.LINK1, archive_folder=test_params.archive)
            test_utils.assert_file_exists(test_utils.LINK2, archive_folder=test_params.archive)
            test_utils.assert_file_exists(
                test_utils.FILE_ABS.lstrip("/"), archive_folder=test_params.archive, use_preffix=False
            )
        elif backend.can_name():
            test_utils.assert_file_exists(test_utils.LINK2)
        else:
    ...
    def test_extract_all_no_folder(self, test_params):
        ...
        base_name = Path(test_params.archive).stem
        test_utils.cleanup(test_utils.DIR)
        test_utils.cleanup(test_utils.FILE1)
        test_utils.cleanup(test_utils.FILE2)
        test_utils.cleanup(test_utils.LINK1)
        test_utils.cleanup(test_utils.LINK2)
        test_utils.cleanup(test_utils.PATH1)
        test_utils.cleanup(test_utils.PATH2)
        test_utils.test_extract(test_params.archive, [], ["-b", test_params.backend, "-a", "-f", "no"])
        if backend.can_pack():
            test_utils.assert_dir_exists(test_utils.DIR)
            test_utils.assert_file_exists(test_utils.FILE1)
            test_utils.assert_file_exists(os.path.join(test_utils.PATH1, test_utils.FILE1))
            test_utils.assert_file_exists(os.path.join(test_utils.PATH2, test_utils.FILE1))
            # https://github.com/p7zip-project/p7zip/issues/39
            if test_params.binary not in (test_utils.X7ZA, test_utils.P7ZIP):
                test_utils.assert_link_exists(test_utils.LINK1)
            test_utils.assert_file_exists(test_utils.LINK2)
            test_utils.assert_file_exists(test_utils.FILE_ABS.lstrip("/"), use_preffix=False)
        elif backend.can_name():
            test_utils.assert_file_exists(test_utils.LINK2)
        else:
            test_utils.assert_file_exists(base_name)
    ...
    def test_extract_encrypted(self, test_params, mocker):
        ...
        base_name = Path(test_utils.ENC_PRE + test_params.archive).stem
        test_utils.cleanup(base_name)
        backend = t_classifier.detect_format(
            test_params.backend, test_utils.ENC_PRE + test_params.archive, t_constants.OPERATION_EXTRACT
        )
        if backend.can_encrypt():
            mocker.patch("rich.prompt.Prompt.ask", return_value=test_utils.PASSWORD)
            test_utils.test_extract(test_utils.ENC_PRE + test_params.archive, [], ["-b", test_params.backend, "-a"])
    ...
    def test_rename(self, test_params):
        ...
        backend = t_classifier.detect_format(
            test_params.backend, test_utils.ENC_PRE + test_params.archive, t_constants.OPERATION_RENAME
        )
        if backend.can_name() and test_params.backend in (t_constants.BACKEND_7ZIP, t_constants.BACKEND_RAR):
            test_utils.test_rename(
                test_params.archive, [test_utils.FILE1, test_utils.FILE_RN], ["-b", test_params.backend]
            )
        else:
            with pytest.raises(SystemExit):
                test_utils.test_rename(
    ...
    def test_delete_occurrence(self, test_params):
        ...
        if backend.can_duplicate() and test_params.backend not in (t_constants.BACKEND_CPIO):
            test_utils.test_delete(test_params.archive, [test_utils.FILE1], ["-b", test_params.backend, "-o", "1"])
    ...
    def test_delete(self, test_params):
        ...
        if backend.can_multiple() and test_params.backend not in (t_constants.BACKEND_CPIO):
            test_utils.test_delete(test_params.archive, [test_utils.FILE2], ["-b", test_params.backend])
        else:
            with pytest.raises(SystemExit):
                test_utils.test_delete(test_params.archive, [test_utils.FILE2], ["-b", test_params.backend])
    ...
    def test_delete_encrypted(self, test_params, mocker):
        ...
        backend = t_classifier.detect_format(
            test_params.backend, test_utils.ENC_PRE + test_params.archive, t_constants.OPERATION_DELETE
        )
        if backend.can_encrypt():  # Packing is available whenever encryption is available
            mocker.patch("rich.prompt.Prompt.ask", return_value=test_utils.PASSWORD)
            test_utils.test_delete(
                test_utils.ENC_PRE + test_params.archive, [test_utils.FILE2], ["-b", test_params.backend]
            )

    def test_cleanup(self, test_params):
        "Not a real test, just the cleanup"

        test_utils.test_cleanup(test_params)
